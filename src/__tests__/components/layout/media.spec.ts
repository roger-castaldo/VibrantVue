import { expect, test,describe } from 'vitest'
import { render } from 'vitest-browser-vue'
import { ExecuteAccessibilityChecks } from '../../common';
import Media from '../../../components/layout/media.vue';
import { stripCommentNodes } from '../../common';
import {h, nextTick, ref} from 'vue';
import { mount } from '@vue/test-utils';

const imageElement = h('img',{alt:'sample image',src:'data:image/png;base64,'});


describe('Media', () => {
  test('check accessibility',async() => {
    const accessibilityScanResults =  await ExecuteAccessibilityChecks(()=>{
      const {container} = render(Media, {
        props: {},
        slots:{
          left_figure:()=>imageElement,
          content:()=>'This is media content',
          right_section:()=>'This is the right side'
        }
      });
      return container;
    });

    expect(accessibilityScanResults.violations).toEqual([]);
  }),
  test('check defaults',async() => {
    const leftContent = 'This is the left side';
    const content = 'This is media content';
    const rightContent = 'This is the right side';
    
    const {container} = render(Media, {
      props: {},
      slots:{
        left_figure:()=>leftContent,
        content:()=>content,
        right_section:()=>rightContent
      }
    });

    const media = container.childNodes[0] as HTMLElement;

    expect(media.className).toBe('media');
    expect(media.tagName).toBe('ARTICLE');

    const children = stripCommentNodes(media);

    expect(children).toHaveLength(3);

    const leftContainer = children[0] as HTMLElement;
    const contentContainer = children[1] as HTMLElement;
    const rightContainer = children[2] as HTMLElement;

    expect(leftContainer.classList).toContain('media-left');
    expect(leftContainer.classList).toContain('is-skeleton');
    expect(leftContainer.innerText).toBe(leftContent);

    expect(contentContainer.classList).toContain('media-content');
    expect(contentContainer.classList).toContain('is-skeleton');
    expect(contentContainer.innerText).toBe(content);

    expect(rightContainer.classList).toContain('media-right');
    expect(rightContainer.classList).toContain('is-skeleton');
    expect(rightContainer.innerText).toBe(rightContent);
  }),
  test('check without left',async() => {
    const content = 'This is media content';
    const rightContent = 'This is the right side';
    
    const {container} = render(Media, {
      props: {},
      slots:{
        content:()=>content,
        right_section:()=>rightContent
      }
    });

    const media = container.childNodes[0] as HTMLElement;

    expect(media.className).toBe('media');
    expect(media.tagName).toBe('ARTICLE');

    const children = stripCommentNodes(media);

    expect(children).toHaveLength(2);

    const contentContainer = children[0] as HTMLElement;
    const rightContainer = children[1] as HTMLElement;

    expect(contentContainer.classList).toContain('media-content');
    expect(contentContainer.classList).toContain('is-skeleton');
    expect(contentContainer.innerText).toBe(content);

    expect(rightContainer.classList).toContain('media-right');
    expect(rightContainer.classList).toContain('is-skeleton');
    expect(rightContainer.innerText).toBe(rightContent);
  }),
  test('check without right',async() => {
    const leftContent = 'This is the left side';
    const content = 'This is media content';
    
    const {container} = render(Media, {
      props: {},
      slots:{
        left_figure:()=>leftContent,
        content:()=>content
      }
    });

    const media = container.childNodes[0] as HTMLElement;

    expect(media.className).toBe('media');
    expect(media.tagName).toBe('ARTICLE');

    const children = stripCommentNodes(media);

    expect(children).toHaveLength(2);

    const leftContainer = children[0] as HTMLElement;
    const contentContainer = children[1] as HTMLElement;

    expect(leftContainer.classList).toContain('media-left');
    expect(leftContainer.classList).toContain('is-skeleton');
    expect(leftContainer.innerText).toBe(leftContent);

    expect(contentContainer.classList).toContain('media-content');
    expect(contentContainer.classList).toContain('is-skeleton');
    expect(contentContainer.innerText).toBe(content);

  }),
  test('check loading setting',async() => {
    const leftContent = 'This is the left side';
    const content = 'This is media content';
    const rightContent = 'This is the right side';

    const props = {
      is_loading: ref(true)
    };
    
    const wrapper = mount(Media, {
      props: props,
      slots:{
        left_figure:()=>leftContent,
        content:()=>content,
        right_section:()=>rightContent
      }
    });

    let media = wrapper.element as HTMLElement;

    expect(media.className).toBe('media');
    expect(media.tagName).toBe('ARTICLE');

    let children = stripCommentNodes(media);

    expect(children).toHaveLength(3);

    let leftContainer = children[0] as HTMLElement;
    let contentContainer = children[1] as HTMLElement;
    let rightContainer = children[2] as HTMLElement;

    expect(leftContainer.classList).toContain('media-left');
    expect(leftContainer.classList).toContain('is-skeleton');
    expect(leftContainer.innerText).toBe(leftContent);

    expect(contentContainer.classList).toContain('media-content');
    expect(contentContainer.classList).toContain('is-skeleton');
    expect(contentContainer.innerText).toBe(content);

    expect(rightContainer.classList).toContain('media-right');
    expect(rightContainer.classList).toContain('is-skeleton');
    expect(rightContainer.innerText).toBe(rightContent);

    props.is_loading.value = false;

    await nextTick();

    media = wrapper.element as HTMLElement;
    children = stripCommentNodes(media);

    leftContainer = children[0] as HTMLElement;
    contentContainer = children[1] as HTMLElement;
    rightContainer = children[2] as HTMLElement;

    expect(leftContainer.classList).toContain('media-left');
    expect(leftContainer.classList).not.toContain('is-skeleton');
    expect(leftContainer.innerText).toBe(leftContent);

    expect(contentContainer.classList).toContain('media-content');
    expect(contentContainer.classList).not.toContain('is-skeleton');
    expect(contentContainer.innerText).toBe(content);

    expect(rightContainer.classList).toContain('media-right');
    expect(rightContainer.classList).not.toContain('is-skeleton');
    expect(rightContainer.innerText).toBe(rightContent);
  })
});