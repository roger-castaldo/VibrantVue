...
<script lang="ts" setup>
    ...
    const Translate = inject<(value:string)=>string>('Translate');
    const Translator = computed<(value:string)=>string>(()=>Translate??function(val){return val;});
    ...
    const inputProps = computed(() => {
        ...
        if (TRANSLATE_FIELDS.some(t=>t===props.input.type)) {
    ...
    const enableField = (values:string[]|string):void=> {
        if (props.input.type === 'subform' && inp.value != null)
            inp.value.enableField(values);
        else
            throw 'unable to call enable field on any form element except a subform';
    };
    const disableField = (values:string[]|string):void=> {
        if (props.input.type === 'subform' && inp.value !== null)
            inp.value.disableField(values);
        else
            throw 'unable to call disable field on any form element except a subform';
    };
    const showField = (values:string[]|string):void=> {
        if (props.input.type === 'subform' && inp.value !== null)
            inp.value.showField(values);
        else
            throw 'unable to call show field on any form element except a subform';
    };
    const hideField =  (values:string[]|string):void=> {
        if (props.input.type === 'subform' && inp.value !== null)
            inp.value.hideField(values);
        else
            throw 'unable to call hide field on any form element except a subform';
    };
    const hideValue = (value:string[]|string):void=> {
        if(inp.value!=null){
            switch (props.input.type) {
                case 'checkbox-group':
                case 'radio-group':
                case 'select':
                    inp.value.hideValue(value);
                    break;
                default:
    ...
    const showValue = (value:string[]|string):void=>{
        if(inp.value!=null){
            switch (props.input.type) {
                case 'checkbox-group':
                case 'radio-group':
                case 'select':
                    inp.value.showValue(value);
                    break;
                default:
    ...
    const disableValue = (value:string[]|string):void=> {
        if(inp.value!=null){
        switch (props.input.type) {
            case 'checkbox-group':
            case 'radio-group':
            case 'select':
                inp.value.disableValue(value);
                break;
            default:
    ...
    const enableValue= (value:string[]|string):void=> {
        if (inp.value!=null){
        switch (props.input.type) {
            case 'checkbox-group':
            case 'radio-group':
            case 'select':
                inp.value.enableValue(value);
                break;
            default:
    ...
    defineExpose({ setValue, fieldName, type,altFieldName, getValue, isValid, setValues, enableField, disableField, showField, hideField, hideValue, showValue, disableValue, enableValue });
</script>
